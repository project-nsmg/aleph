...
function loadStarData( dataFile, callback ){
    ...
    utils.setLoadMessage("Fetching stellar data");
    xhr.addEventListener( 'load', function ( event ) {
        var parsed = JSON.parse( xhr.responseText );
        // console.log(parsed);
        if( callback ){
            utils.setLoadMessage("Parsing stellar data");
            callback(parsed);
        }
    }, false );
...
function generateHipparcosStars(){
    ...
    for( var i=0; i<count; i++ ){
        ...
        */
        ...
        p.spectralLookup = utils.map(star.c, -0.3, 1.52, 0, 1);
    ...
    for( var i in pGeo.vertices ){
        ...
        if( p.name.length > 0 ){
            ...
            preview.update = function(){
                this.material.opacity = utils.constrain( Math.pow(glworld.camera.position.z * 0.002,2), 0, 1);
                ...
                if( this.material <= 0.0 )
                    ...
                    this.visible = true;
                this.scale.setLength( utils.constrain( Math.pow(glworld.camera.position.z * 0.001,2), 0, 1) )
            }
    ...
    var shaderMaterial = new THREE.ShaderMaterial( {
        ...
        vertexShader:   shaders.datastars.vertex,
        fragmentShader: shaders.datastars.fragment,
    ...
    starBases.update = function(){
        this.material.opacity = utils.constrain( (glworld.camera.position.z - 400.0) * 0.002, 0, 1);
    ...
    container.update = function(){
        var blueshift = (glworld.camera.position.z + 5000.0)/60000.0;
        blueshift = utils.constrain( blueshift, 0.0, 0.2 );

        var brightnessScale = utils.constrain( 10/ Math.sqrt(glworld.camera.position.z), 0, 1);
        ...
        datastarUniforms.zoomSize.value = utils.constrain(( glworld.camera.position.z ) / 4000,0,1);

        var areaOfWindow = window.innerWidth * window.innerHeight;
        datastarUniforms.scale.value = Math.sqrt(areaOfWindow) * 1.5;

        if( glworld.camera.position.z < 1500 ){
    ...
    lineMesh.update = function(){
        ...
        //      some basic LOD
        if( glworld.camera.position.z < 250 )
            this.visible = false;
        else
            this.visible = true;
    }
