...
var addLensFlare = lensflare.addLensFlare;
...
shaders.ready(function() {
    glworld.ready(function() {
        var pGalacticSystem = generateGalaxy();
        glworld.translating.add(pGalacticSystem);

        var pDustSystem = generateDust();
...
function generateGalaxy() {
    var galacticShaderMaterial = new THREE.ShaderMaterial( {
        ...
        vertexShader: shaders.galacticstars.vertex,
        fragmentShader: shaders.galacticstars.fragment,
    ...
    for( var i=0; i<count; i++ ){
        ...
        var thickness = constrain( Math.pow(
            constrain(90-distanceToCenter*0.1,0,100000),2) * 0.02,2,10000) +
            Math.random() * 120;
    ...
    pGalacticSystem.update = function(){
        ...
        glworld.galacticCentering.position.set(0,0,0);
        glworld.galacticCentering.position.lerp(this.position.clone().negate(), targetLerp);
    }
...
function generateDust(){
	...
	var dustShaderMaterial = new THREE.ShaderMaterial( {
		...
		vertexShader:   shaders.galacticdust.vertex,
		fragmentShader: shaders.galacticdust.fragment,
	...
	pDustSystem.update = function(){
		...
		dustUniforms.cameraPitch.value = glworld.rotating.rotation.x;
